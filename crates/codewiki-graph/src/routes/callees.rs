...
pub async fn handle(
    ...
    Path(id): Path<String>,
    Query(params): Query<CalleesParams>,
) -> impl IntoResponse {
    ...
    match result {
        Ok(pairs) => {
            ...
            let items: Vec<CalleeItem> = pairs
                .into_iter()
                .take(limit)
                .map(|(node, edge)| CalleeItem { node, edge })
                .collect();
            Json(CalleesResponse {
                items,
                truncated,
                total,
            })
